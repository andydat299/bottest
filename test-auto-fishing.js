...
async function testFishingCalculations() {
  ...
  try {
    ...
    // Display fish breakdown
    if (Object.keys(results.fishByType).length > 0) {
      console.log('   Fish breakdown:');
      for (const [fishName, data] of Object.entries(results.fishByType)) {
        console.log(`     ${data.emoji} ${fishName}: ${data.count} con (${data.totalValue.toLocaleString()} xu)`);
      }
    }
...
async function testConfiguration() {
  ...
  try {
    ...
    const { fishTypes } = await import('./utils/fishTypes.js');
    ...
    console.log('   Fish Types loaded:');
    console.log(`     Total fish types: ${fishTypes.length}`);
    ...
    for (const [rarity, count] of Object.entries(rarityCount)) {
      console.log(`     ${rarity}: ${count} types`);
    }
