...
/**
 * Handle marriage-related button interactions
 */
export async function handleMarriageButtons(interaction) {
  try {
    ...
    console.error('Error handling marriage buttons:', error);
    
    if (!interaction.replied && !interaction.deferred) {
      await interaction.reply({
        content: '❌ **Có lỗi xảy ra khi xử lý marriage interaction!**',
        ephemeral: true
      });
    }
  }
...
async function handleProposalAccept(interaction) {
  ...
  try {
    ...
    console.error('Error handling proposal accept:', error);
    await interaction.followUp({
      content: '❌ **Có lỗi xảy ra khi xử lý lời cầu hôn!**',
      ephemeral: true
    });
  }
}
  ...
  try {
    ...
    await usersCollection.updateOne(
      { 
        ...
        'ringInventory.id': proposal.ringId 
      },
    ...
    console.error('Error handling proposal reject:', error);
    await interaction.followUp({
      content: '❌ **Có lỗi xảy ra khi xử lý lời cầu hôn!**',
      ephemeral: true
    });
...
async function handleDivorceConfirm(interaction) {
  ...
  try {
    ...
    const marriage = await marriagesCollection.findOne({ 
      _id: new mongoose.Types.ObjectId(marriageId) 
    });
    ...
    try {
      await session.withTransaction(async () => {
        ...
        await usersCollection.updateOne(
          { 
            discordId: { $in: [marriage.partner1, marriage.partner2] }, 
            'ringInventory.id': marriage.ringId 
          },
    ...
    console.error('Error handling divorce confirm:', error);
    await interaction.followUp({
      content: '❌ **Có lỗi xảy ra khi xử lý ly hôn!**',
      ephemeral: true
    });
  }
}
